...
public class ContactDaoImpl implements ContactDao {
	...
	// singleton step 2   // fields 변수 선언 후 생성자 
	private ContactDaoImpl() {
		dataDir = FileUtil.initDataDir(); // static이라 class명.메서드 호출 가능 // 데이터 폴더 초기화
		dataFile = new File(FileUtil.DATA_DIR, FileUtil.DATA_FILE);
		contacts = FileUtil.initData();	// 데이터 초기화 - 파일의 내용을 메모리에 로딩. // = new ArrayList<>();
	}
	
	//TODO: ContactDaoImpl에는 연락처 데이터를 변경하는 메서드들이 있음. 새연락처.수정.삭제
	//-> 연락처 데이터가 변경되는 메서드에서 FileUtil.writeDataToFile() 메서드를 호출.
	 ...
	 * @return 사용가능한(유효한) 인덱스는 true, 그렇지 않으면  false.
	 */
	...
	public int create(Contact c) {
		contacts.add(c); // 메모리의 List에 연락처를 추가. 파일xx Ctrl마우스 클릭하면 필드 선언했으나 초기화 안돼있음. 그 위에보면 생성자에서 초기화 함.
        //-> (heap) 메모리에 있는 List에 연락처를 추가.
		FileUtil.writeDataToFile(contacts, dataFile); // 파라미터 - ArrayList, new File(FileUtil.DATA_DIR, FileUtil.DATA_FILE)
