...
public interface ContactDao {
	 ...
	 */
	int create(Contact c); // == int create(String name, String phone, String email);
	
	
	/**
	 * 연락처 전체 목록 보기 기능. 저장된 연락처 개수 크기의 연락처 리스트 리턴.  
	 * @return 연락처 배열(Contact[]). --- 연락처 리스트 return타입 바뀜. List<Contact> read();
	 * 
	 */
	List<Contact> read();
	...
	/**
	 * 인덱스 검색. 전달받은 인덱스 위치에 있는 연락처 객체를 리턴. --------리스트에 저장 인덱스 XXX
	 * @param index 검색할 인덱스. 0 이상의 정수.					--------리스트에 저장
	 * @return 해당 인덱스의 연락처(Contact) 객체 또는 null.		--------리스트에 저장
	 */
