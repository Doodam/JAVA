...
public class ContactMain04 {
	...
	private Scanner scanner = new Scanner(System.in); // 변경 못하게 final사용 가능.
	private ContactDaoImpl dao = ContactDaoImpl.getInstance(); // private로 감췄기 때문에 객체를 호출할 때는 static 메서드로 (.)로 호출, 변경 못하게 final사용 가능.

	public static void main(String[] args) {
		System.out.println("*** 연락처 프로그램 v0.4 ***");
	
		
		ContactMain04 app = new ContactMain04(); // ContactMain03의 static이 아닌 필드와 메서드들을 사용하기 위해서.
	...
	private void deleteContact() {
		System.out.println("\n----- 연락처 삭제 -----");
		System.out.println("삭제할 연락처의 인덱스 입력>> ");
	...
	private void updateContact() {
		System.out.println("\n----- 연락처 업데이트 -----");
		System.out.println("수정할 연락처의 인덱스 입력>> ");
		int index = inputNumber();
	...
	private void selectContactByIndex() {
		...
		// View에서는 Controller의 메서드를 호출해서 인덱스 검색 결과를 리턴 받음.
	...
	private void selectAllContacts() {
		...
		List<Contact> contacts = dao.read();// View(UI)는 Controller의 메서드를 호출해서 출력에 필요한 데이터 리턴받음.
...
//		}
		...
		int result = dao.create(contact); // 컨트롤러 객체의 메서드를 호출해서 연락처 저장 기능을 실행.
		if (result == 1) {
	...
	private int showMainMenu() {
		System.out.println();
		System.out.println("----------------------------------------------------------------");
		System.out.println("[0]종료 [1]새 연락처 [2]전체 목록 [3]인덱스 검색 [4]수정 [5]삭제");
		System.out.println("----------------------------------------------------------------");
		System.out.print("선택>> ");
		int n = inputNumber();
