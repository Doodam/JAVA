 ...
 * 
 * ArrayList <---> LinkedList 장/단점 반대
 * 
 * LinkedList의 특징:
 * 1. linked list 알고리즘을 사용한 리스트.
 * 2. 데이터를 저장(add)하거나 삭제(remove)할 때 속도가 빠름.
 * 3. 데이터를 검색(get)할 때 속도가 느림.
 * 
 * Collection<E>
 * |__ List<E>
 * 	   |__ ArrayList<E>, LinkedList<E>, ... <> ~타입의 원소를 갖는지
 */
...
public class ListMain01 {
	...
	public static void main(String[] args) {
		...
		languages.add("HTML");
		languages.add("Java");
		System.out.println("리스트 크기: " + languages.size()); // 개수
		System.out.println(languages); // [원소이름]
		...
		// remove(Object o): 리스트에서 처음 등장하는 원소를 찾아서 1개를 삭제.
		// remove(int index): 리스트에서 인덱스 위치에 있는 원소를 1개 삭제.
		...
		while(itr.hasNext()) { // 다음 원소가 있는지?
			System.out.println(itr.next()); // 있으면 한칸만 출력후 이동
		}
