...
public class FileMain04 {
	...
	public static void main(String[] args) {
		...
		try (	// close 메서드를 호출해야 하는 !!변수들만!!(=생성자만!) 선언, 초기화 --> close는 try-catch가 끝난 후 자동으로 호출.
				FileInputStream in = new FileInputStream(origin); 
				FileOutputStream out = new FileOutputStream(dest);
		  ) { 
			while(true) {
				byte[] buf = new byte[10]; // 파일에서 읽은 내용을 저장할 byte 배열
				int read = in.read(buf); // 파일에서 읽은 내용을 배열 buf에 저장하고, 실제로 읽은 바이트 수를 리턴.
				System.out.println(read + "바이트 읽음.");
				if(read == -1) { // EOF
					break;
				}
				out.write(buf, 0, read);
				// 배열 buf의 인덱스 0번 위치부터 길이 read 만큼을 파일에 씀.
			}
