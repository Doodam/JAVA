 ...
 *  InputStreamReader, OutputStreamWriter 가 인코딩 설정 기능을 가지고 있었음.
 *  Java 11 이후 버전부터는 FileReader와 FileWriter에서도 인코딩 설정을 할 수 있게 업데이트 됨., ㅡ.
 *     
 * 인코딩(encoding) : 문자에 해당하는 코드(정수)로 변환. (예) 'A' -> 65
 * 디코팅(decoding) : 문자 코드(정수)에 해당하는 문자로 변환. (예) 65 -> 'A'
...
public class FileMain08 {
	...
	public static void main(String[] args) {
		...
		try(
			 ...
			 // 3. 메모리 버퍼를 사용해서 읽기 속도를 빠르게 하기 위해서 (옵션)
		...
		// 1. Java 11 버전부터 문자 스트림을 사용하는 방법 :
		// FileReader, FileWriter에서 인코딩 방식을 설정할 수 있게됨.
		try(
				// 1. FileReader를 생성할 때 인코딩 방식을 설정.j
...
//				System.out.print((char) read); // 읽은 1글자를 콘솔에 출력
				String line = br.readLine(); // 1줄씩 읽는 메서드.
				if(line == null) { // EOF
					break;
				}
