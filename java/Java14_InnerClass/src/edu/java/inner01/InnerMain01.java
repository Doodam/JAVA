 ...
 * 	(1) 인스턴스 필드(변수) : static이 아닌 필드.
 * 		- 객체를 생성한 후에, 그 참조 변수로 접근해서 사용하는 변수.
 * 	(2) 클래스 필드(변수) : static으로 선언된 필드.
...
public class InnerMain01 {
	public static final int MAX = 5; // 클래스(static) 변수 -> 메모리 Method영역    //다른 클래스에서 가져오고 싶을 땐 InnerMain01.MAX
	...
	public static void main(String[] args) {
		...
		InnerMain01 app = new InnerMain01(); // 생성자 호출 시 인스턴스 필드(변수) 메모리 heap에 생성.
		System.out.println("x = " + app.x); // app : 참조변수
...
//		Enclosing.Nested nested = new Enclosing.Nested(2); // import문 없을 때
		Nested nested = new Nested(2); // import문 있을때
		nested.printInfo();
		...
		Outer outer1 = new Outer(100, 80, "abc");
		System.out.println(outer1);
		
		// 내부 클래스 Inner 타입의 객체를 생성.
		// 인스턴스는 내부 클래스는 참조 변수를 사용해야 생성자를 호출할 수 있음.
//		Outer.Inner1 = outer1.new Inner(200);	// import문 없는 경우
		
		Inner inner = outer1.new Inner(200);  //import 문장을 사용한 경우:
		inner.printInfo();
