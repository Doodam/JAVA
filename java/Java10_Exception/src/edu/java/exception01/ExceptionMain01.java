...
// error의 종류:
// 1. 시스템 에러: (예) OOM(Out-of-Memory, 메모리부족) -> 수정 불가능. 수정 힘듦.
// 2. 컴파일 에러: 문법 오류 -> 반드시 수정!
// 3. 논리적인 오류: 프로그램이 정상적으로 컴파일 되고 실행/종료가 되지만, 원하는 결과가 나오지 않은 경우.
//   ->> 반드시 수정. 수정 힘든 경우가 많음.
...
//		System.out.println("x / y = " + (x / y)); // 나누기 0을 하면 오류 발생 --> 예외(exception) 
		// if-else 구문 사용.
		if (y != 0) {
