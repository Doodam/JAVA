...
// final + 지역변수: 변경할 수 없는 지역 변수. 상수(constant). 초기화 이후에는 값을 변경할 수 없는 변수.
// final + 클래스 멤버(field,constructor,method): 변경할 수 없는 클래스 멤버(필드, 메서드).
// final + 필드(멤버변수)는 반드시 초기화하는 문장을 명시해야 함.
//  (1) 필드 선언과 동시에 값을 초기화.
...
public class ModifierMain06 {
	...
	private static final int VERSION = 1; // (1) 필드 선언과 동시에 값을 초기화.
