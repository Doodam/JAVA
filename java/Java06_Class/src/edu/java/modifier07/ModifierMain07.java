...
// - 프로그램 로딩 시점(main 메서드 시작 전)에 JRE이 사용하는 메모리 공간 중에서 메서드(method) 영역에 생성됨. 사라지지 않아서 용량 많이 차지함!
// - 클래스 이름을 사용해서 사용하는 멤버.
// - static메서드는 static멤버(필드,메서드)들만 사용 가능.
// -> static 메서드는 인스턴스멤버(필드,메서드)를 사용할 수 없음.
...
public class ModifierMain07 {
	...
	public static void main(String[] args) {
		...
		test1();	// static 메서드는 static멤버(필드,메서드)를 사용할 수 있음.
//		test2();	// static 메서드에서 instance 멤버(필드,메서드)를 사용할 수 없음.
		ModifierMain07 app = new ModifierMain07(); // test2 호출 하려면 ModifierMain07타입 객체를 먼저 생성!!
		app.test2();
