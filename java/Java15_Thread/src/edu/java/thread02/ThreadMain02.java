...
public class ThreadMain02 {
	...
	public static void main(String[] args) {
		 ...
		 * 	  new Thread(Runnable runnable) 생성자 호출 -> Thread 객체 생성.
		...
		class MyRunnable implements Runnable {
			...
			public void run() {
				for(int i = 0; i < 100; i++) {
					...
					try {
						Thread.sleep(10); // static 메서드이기 때문에 Thread 정의하지 않아도 사용 가능
		...
		try {
			th1.join(); // 실행된 Thread가 끝날 때 까지 main 메서드는 기다림.
			th2.join();
			th3.join();
		} catch (InterruptedException e) {
			e.printStackTrace();
		...
		long end = System.currentTimeMillis();	// Thread 종료
