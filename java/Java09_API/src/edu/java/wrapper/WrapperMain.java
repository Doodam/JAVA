...
public class WrapperMain {
	...
	public static void main(String[] args) {
		int x = 1; // 기본 타입(primitive type) - 메서드 없음. (ex -> x.메서드 안뜸)
		
		// Integer 타입의 객체 생성 - 생성자는 권장하지 않고, static 메서드 사용권장.
		...
		// auto-boxing (wrapping): int 타입 값을 Integer타입 객체로 자동 변환.
		// Integer.valueOf(1) 코드가 자동으로 실행.
		Integer number3 = 1; // Integer number3객체에 1을 넣음.
		...
		int num = Integer.parseInt("100"); // 리턴타입 Integer(객체)와 int(기본타입) 차이
		...
		double d1 = 1.1;
		double d2 = 0.9;
		System.out.println(d1 - d2); // 1.1 - 0.9 = 0.2,,, 출력은 0.20000000000000007
...
//		System.out.println(big1 - big2); // Wrapper class가 아니기 때문에 안됨.
		System.out.println(big1.subtract(big2));
		// BigDecimal 타입의 객체는 메서드를 호출해서 산술 연산(+, -, *, /)을 수행.
		// BigDecimal 클래스는 기본 타입의 Wrapper클래스가 아니기 때문에 auto-boxing/unboxing 기능이 없음.
