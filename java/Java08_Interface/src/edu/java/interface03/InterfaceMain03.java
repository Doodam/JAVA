...
class H extends A implements E, F {} // 1개의 클래스를 상속하고 2개의 인터페이스를 구현하는 클래스.
...
interface Seller {
	...
	// default (인스턴스) 메서드. 객체가 생성 될때 사용 가능.
	default void register() {
...
class User implements Buyer, Seller {
	...
	// 중복되는 default만 (static xx) 메서드가 2개의 인터페이스에 있는 경우에는 반드시 Override.
	@Override
	public void register() {
		// 상위인터페이스.super.메서드() 호출
		Buyer.super.register(); // Buyer인터페이스의 default 메서드가 호출됨.
		Seller.super.register(); // Seller인터페이스의 default 메서드가 호출됨.
